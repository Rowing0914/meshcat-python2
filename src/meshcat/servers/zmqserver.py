...
DEFAULT_FILESERVER_PORT = int(os.getenv("DEFAULT_FILESERVER_PORT", 7000))
MAX_ATTEMPTS = 1000
DEFAULT_ZMQ_METHOD = "tcp"
DEFAULT_ZMQ_PORT = int(os.getenv("DEFAULT_ZMQ_PORT", 6000))
...
class ZMQWebSocketBridge(object):
    ...
    def __init__(self, zmq_url=None, web_url=None, host="127.0.0.1", port=None,
        ...
        if web_url is None:
            self.web_url = "{protocol}//{host}:{port}/static/".format(protocol=protocol, host=self.host, port=self.fileserver_port)
        else:
...
def main():
    ...
    parser.add_argument('--web_url', type=str, default=None)
